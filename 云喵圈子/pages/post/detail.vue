		...
		<view class="info-box">
			...
			<view class="post-content-wrap">
				<navigator class="discuss-item" :url="'/pages/discuss/detail?title='+item"
					v-for="(item,index) in discussList" :key="index">#{{item}}#</navigator>
				<text class="post-text" @longpress="onCopy">{{postDetail.content}}</text>
			...
			getPostDetail() {
				this.$H
					...
					.then(res => {
						...
						//渲染话题链接
						// let discussText = "<navigator url='pages/index' style='color: dodgerblue;'>$&</navigator>";
						let discussList = res.result.content.match(/#(.*?)#/g);
						...
						res.result.content = res.result.content.replace(/#(.*?)#/g, '');
