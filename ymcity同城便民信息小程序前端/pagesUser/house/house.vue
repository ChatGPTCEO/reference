<template>
	<view class="pageCont">
		<u-tabs :list="list" :is-scroll="false" active-color="#f44c01" :current="current" @change="change"></u-tabs>
		<z-paging ref="paging" @query="queryList" :list.sync="listData" :style="{height: scrollHeight+'px'}">
			...
			<view>
				<view class="list" v-for="(item,index) in listData" :key="index">
					<view class="titbox" @click="$u.route('/pagesIndex/housemain/housemain',{id:item.id})">
						<u-icon v-if="item.is_top=='2'" name="/static/images/top.png" size="40"
							style="margin-right: 10rpx;"></u-icon>
						<text class="title">{{item.title}}</text>
					</view>
					<text class="type" @click="$u.route('/pagesIndex/housemain/housemain',{id:item.id})">{{item.type_text}}</text>
					<view class="cell" @click="$u.route('/pagesIndex/housemain/housemain',{id:item.id})">
						<text class="price" v-if="item.info_type=='sell'">{{item.rent}}万元</text>
						<text class="price" v-else>{{item.rent}}元/月</text>
					...
					<view class="tools">
						...
						<view v-if="item.status=='5'" @click="topay(item)" class="btn" style="background-color: #FF9900;color: white;">
							<text>支付</text>
						</view>
	...
	export default {
		...
		methods: {
			...
			queryList(pageNo, pageSize) {
				...
				this.$u.get('/addons/hicity/house/pushList', this.search).then(res => {
			...
			toTop(params) {
				this.$u.get('/addons/hicity/house/topInfo', params).then(res => {
			...
			downInfo(item, index) {
				this.$u.get('/addons/hicity/house/upInfoStatus', {
					id: item.id,
					type: 'down'
				}).then(res => {
			...
			topay(item){
				this.$u.get('/addons/hicity/house/topay', {
					id: item.id,
				}).then(res => {
					this.$u.route('/pagesPush/pay/pay', {
						order_no: res.order_no,
