<template>
	<view>
		...
		<!-- 发布按钮 -->
		<view @click="handelAdd" class="plus-box"><u-icon color="#fff" name="plus"></u-icon></view>
		<!-- 发布弹窗 -->
...
export default {
	...
	methods: {
		...
		onTrigger(type) {
			...
			if (type == 3) {
				uni.navigateTo({
					url: '/pages/post/vote?topic_id=' + this.discussInfo.topic_id+'&discuss_id='+ this.discussInfo.id+ '&dis_name=' + this.discussInfo.title
				});
			} else {
				uni.navigateTo({
					url: '/pages/post/add?topic_id=' + this.discussInfo.topic_id + '&type=' + type+'&discuss_id='+ this.discussInfo.id+ '&dis_name=' + this.discussInfo.title
				});
			}
		},
		getDiscussInfo() {
		...
		getPostList() {
			...
			this.$H
				.get('post/list', {
					discuss_id: this.discussInfo.id,
					page: this.page
				})
				.then(res => {
