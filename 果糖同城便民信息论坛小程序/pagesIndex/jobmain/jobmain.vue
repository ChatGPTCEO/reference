	...
	<view>
		<view v-if="main">
			<view class="jobcard">
				...
				<text class="price" v-if="main.is_face=='0'">{{main.salary_min}}-{{main.salary_max}}元/月</text>
				<text class="price" v-else>面议</text>
				<view class="info-tag">
					<u-image width="28rpx" height="28rpx" src="@/static/images/job.png"></u-image>
					<text style="margin-left: 10rpx;">{{main.jobcategory.name}}</text>
				</view>
				<view class="tagbox">
					<view class="info-tag">
						<u-image width="28rpx" height="28rpx" src="@/static/images/exp.png"></u-image>
						<text style="margin-left: 10rpx;" v-if="main.work_exp=='0'">经验不限</text>
						<text style="margin-left: 10rpx;" v-else>{{main.work_exp_text}}经验</text>
					</view>
					<view class="info-tag" style="margin-left: 30rpx;">
						<u-image width="28rpx" height="28rpx" src="@/static/images/edu.png"></u-image>
						<text style="margin-left: 10rpx;" v-if="main.edu=='0'">学历不限</text>
						<text style="margin-left: 10rpx;" v-else>{{main.edu_text}}学历</text>
					</view>
				</view>
				<view class="bot">
					<!-- <text>98次浏览</text> -->
					<text>{{main.createtime_text}}</text>
				</view>
			</view>
			<view class="labels">
				<text class="label" v-for="(item,index) in main.labelconfig" :key="index">{{item.name}}</text>
			</view>
			<view class="cont">
				<text class="cont-title">岗位描述</text>
				<text class="cont-text">{{main.content}}</text>
				<text class="cont-text">联系我时请告知是在【{{appName}}】信息服务平台看到的！</text>
			</view>
			<view class="cont" style="margin-top: 40rpx;">
				<text class="cont-title">工作地址</text>
				<view class="sitebox">
					<view class="address">
						<u-icon name="map" color="#40c6bf" size="30"></u-icon>
						<text class="text">{{main.work_address}}</text>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			getMain() {
				this.$u.get('/addons/hicity/job/main', {
					id: this.id
				}).then(res => {
					this.main = res;
				})
