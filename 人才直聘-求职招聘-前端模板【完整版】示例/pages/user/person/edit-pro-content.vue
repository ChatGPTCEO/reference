...
export default {
	...
	methods: {
		async register() {
			...
			if(this.type==1){
				this.$mPage.prePage().descr = this.content;
			}else if(this.type==2){
				this.$mPage.prePage().performance = this.content;
			}else if(this.type==3){
				this.$mPage.prePage().content = this.content;
			}else if(this.type==4){
				this.$mPage.prePage().company.companyProfile = this.content;
			}else if(this.type==5){
				this.$mPage.prePage().position.descr = this.content;
			}
			uni.navigateBack({
				delta:1
			})
