              ...
              <view v-if="postComments.length > 0">
                <view v-for="(commentPost, index) in postComments" :key="index">
                  <qui-topic-comment
                    ...
                    :comment-type="1"
                    :post-id="commentPost.id || (commentPost._jv && commentPost._jv.id)"
                    :comment-user="
                    ...
                    :is-real="commentPost.user.isReal || commentPost.user.isRealName"
...
export default {
  ...
  methods: {
    ...
    postOpera(id, type, canStatus, isStatus, commentPost) {
      ...
      if (type == '1' || type == '4') {
        apiRequest.updatePosts({ data: params }).then(data => {
          if (type == '1') {
      ...
      apiRequest.createPosts({ data: params })
        .then(res => {
          ...
          if (res.Data.isApproved == 1) {
            ...
            this.$u.event.$emit('addComment', { data: res.Data, commentId: this.commentId });
          } else {
