...
export default {
  ...
  async onShareAppMessage(res) {
    ...
    if (res.from === 'button' && res.target.id !== 'top') {
      const params = {
        pid: Number(this.nowThreadId),
      };
      const threadShare = await newApi.readThreadDetail({ params });
      return {
        title:
          threadShare.Data.thread.type === 1
            ? threadShare.Data.thread.title
            : threadShare.Data.firstPost.summaryText,
