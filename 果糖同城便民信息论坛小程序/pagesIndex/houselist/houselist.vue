		...
		<z-paging ref="paging" @query="queryList" :list.sync="houseData" :style="{height: scrollHeight+'px'}">
			<view slot="top">
				<u-dropdown style="background-color: white;" ref="uDropdown" active-color="#f44c01">
					<u-dropdown-item v-model="search.room" title="户型" :options="options1" @change="refload">
					</u-dropdown-item>
					<u-dropdown-item v-model="search.rent" title="租金" :options="options2" @change="refload">
					</u-dropdown-item>
							...
							<view class="slot-cell">
								...
								<view class="gird">
									<view class="gitem" v-for="(item,index) in acreageTag" :key="index"
										@click="seltag('acreage',item)"
										:class="{active:search.acreage==item.value?true:false}">
							...
							<view class="slot-cell">
								...
								<view class="gird">
									<view class="gitem" v-for="(item,index) in labelTag" :key="index"
										...
										:class="{active:search.lable_ids==item.value?true:false}">
										<text>{{item.label}}</text>
									</view>
								</view>
							</view>
