					...
					<view class="tx">
						<view class="tx-grid">
							<view class="tx-grid-text">
								<view class="title">可使用花瓣</view>
								<view class="money">
									<text>{{bydata ? bydata.pay:0}}</text>
									<text class="money-b">瓣</text>
								</view>
							</view>
						</view>
						<!-- <view class="tx-grid" @click="navTo('/pages/user/qian')"><span class="tx-grid-comm-sign">立即提现</span></view> -->
					</view>
					<view class="palace palace-one">
						<view class="palace-grid">
							<view class="palace-grid-text">
								<view class="palace-grid-text-data">
									...
									<text class="palace-grid-text-data-b">瓣</text>
								</view>
								<view class="palace-grid-text-name">累计使用</view>
							</view>
						</view>
						<view class="palace-grid">
							<view class="palace-grid-text">
						...
						<view class="palace-grid">
							<view class="palace-grid-text">
								...
								<view class="palace-grid-text-name">累计充值</view>
							</view>
						</view>
					</view>
				</view>
					...
					<view class="com-wrap">
						<view class="cell" v-for="(item, index) in list" :key="index">
							<view class="cell-left">
								<view class="cell-text">{{index+1}},{{item.ctime|timestampToTime}}-{{ item.title}}</view>
							</view>
							<view class="pay" v-if="current==0">+{{item.pay}}</view>
							<view class="pay" v-if="current==1">-{{item.pay}}</view>
							<view class="pay" v-if="current==2">+{{item.pay}}</view>
						</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		getlist() {
			...
			uni.$u.http.post('Detailed/payData', {id:this.userInfo.id}).then(res => {
				uni.hideLoading();
				// console.log(res)
		...
		getpaylist() {
			// uni.showLoading({title: '数据加载中',mask:true});
			uni.$u.http.post('Detailed/payList', {id:this.userInfo.id,current:this.current}).then(res => {
				// uni.hideLoading();
				// console.log(res)
				if(res.code==200){
					this.list=res.data
