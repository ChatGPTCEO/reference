	...
	<view class="warp" v-if="main">
		...
		<view class="info">
			...
			<u-grid :col="3" :border="false">
				<u-grid-item>
					<view class="grid-params">{{main.price}}万元</view>
					<view class="grid-text">售价</view>
				</u-grid-item>
				<u-grid-item>
					<view class="grid-params">{{main.reg_time_text}}</view>
					<view class="grid-text">上牌时间</view>
				</u-grid-item>
				<u-grid-item>
					<view class="grid-params">{{main.mileage}}万公里</view>
					<view class="grid-text">行驶里程</view>
				</u-grid-item>
			</u-grid>
		</view>
		<view class="content">
...
<script>
	...
	export default {
		...
		methods: {
			getMain() {
				this.$u.get('/addons/hicity/car/main', {
					id: this.id
				}).then(res => {
					this.main = res;
				})
