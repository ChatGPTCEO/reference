	...
	<view class="content">
		...
		<view class="zhuige-article-swiper" v-if="topic && topic.images && topic.images.length > 0">
			<swiper
				...
				autoplay="autoplay"
				circular="ture"
				:style="{ width: '750rpx', height: slideHeight }"
			...
			>
				<swiper-item v-for="(item, index) in topic.images" :key="index">
					<view @click="clickTopicImages(topic.images, index)"><image mode="widthFix" :src="item.image.url"></image></view>
				</swiper-item>
			</swiper>
		</view>
		...
		<view class="zhuige-detail-block">
			<view v-if="topic" class="zhuige-block">
				...
				<view v-if="topic.author" class="zhuige-social-poster-blcok">
					<view @click="openLink('/pages/user/home/home?user_id=' + topic.author.user_id)" class="zhuige-social-poster">
						<view class="zhuige-social-poster-avatar"><image mode="aspectFill" :src="topic.author.avatar"></image></view>
						<view class="zhuige-social-poster-info">
							<view>
								<text>{{ topic.author.nickname }}</text>
								<image v-if="topic.author.vip" mode="aspectFill" src="/static/vv-1.png"></image>
								<image v-if="topic.author.certify && topic.author.certify.status == 1" mode="aspectFill" :src="topic.author.certify.icon"></image>
							</view>
							<view>
					...
					<view class="zhuige-social-opt">
						<view v-if="is_report" @click="openLink('/pages/plugins/report/report?post_id=' + topic.id)"><uni-icons type="more-filled" size="20"></uni-icons></view>
				...
				<view v-if="topic.top_img_ad" class="zhuige-wide-image-ad">
					<view @click="openLink(topic.top_img_ad.link)"><image mode="aspectFill" :src="topic.top_img_ad.image"></image></view>
				</view>

				<!-- 主内容区 -->
				<view class="zhuige-detail-cont"><mp-html :content="topic.content" /></view>

				<!-- 话题 -->
				<view v-if="topic.subjects && topic.subjects.length > 0" class="zhuige-detail-tags">
					<view v-for="(subject, index) in topic.subjects" :key="index" @click="openLink('/pages/bbs/list/list?subject_id=' + subject.id + '&title=' + subject.name)">
				...
				<!-- 地址信息 -->
				<view v-if="topic.location && topic.location.marker" @click="clickLocation" class="zhuige-social-address">
					<view>
				...
				<view v-if="topic.forum" class="zhuige-detail-classify">
					<view class="zhuige-classify-block" @click="openLink('/pages/bbs/forum/forum?forum_id=' + topic.forum.id)">
						<view><image mode="aspectFill" :src="topic.forum.logo"></image></view>
		...
		<view v-if="topic && topic.rec_ad" class="zhuige-detail-ad">
			<zhuige-scroll-ad ext-ad-class="zhuige-scroll-goods-mini" :title="topic.rec_ad.title" :items="topic.rec_ad.items"></zhuige-scroll-ad>
		</view>
			...
			<view class="zhuige-block">
				<view v-if="topic.like_list && topic.like_list.length > 0" class="zhuige-detail-share-user">
					...
					<view class="zhuige-detail-share-user-list">
						<view v-for="(user, index) in topic.like_list" :key="index" @click="openLink('/pages/user/home/home?user_id=' + user.user_id)">
		...
		<view v-if="topic.weixin_ad" class="zhuige-traffic-ad">
			<view class="zhuige-block"><ad-custom :unit-id="topic.weixin_ad"></ad-custom></view>
		</view>
		<!-- #endif -->
		...
		<view class="zhuige-reply-list">
			<view class="zhuige-block">
				...
				<template v-if="topic && topic.comments.length > 0">
					<zhuige-reply v-for="(comment, index) in topic.comments" :key="index" :item="comment" @clickReply="clickReply"></zhuige-reply>
		...
		<view v-if="topic.bottom_img_ad" class="zhuige-wide-image-ad">
			<view @click="openLink(topic.bottom_img_ad.link)"><image mode="aspectFill" :src="topic.bottom_img_ad.image"></image></view>
		</view>
		...
		<view v-if="topic" class="zhuige-detail-comment">
			<view @click="clickComment(0)" class="zhuige-detail-input"><text>我想说两句...</text></view>
			<view class="zhuige-detail-opt">
				<view @click="clickComment(0)">
					<uni-badge :offset="[0, 12]" :text="topic.comment_count.toString()" :inverted="!topic.is_comment" type="error" absolute="rightTop">
						<uni-icons :type="topic.is_comment == 1 ? 'chat-filled' : 'chat'" :color="topic.is_comment == 1 ? '#fd6531' : '#444444'" size="28"></uni-icons>
					</uni-badge>
				</view>
				<view @click="clickLike">
					<uni-badge :offset="[0, 12]" :text="topic.like_list.length.toString()" :inverted="!topic.is_like" type="error" absolute="rightTop">
						<uni-icons :type="topic.is_like == 1 ? 'hand-up-filled' : 'hand-up'" :color="topic.is_like == 1 ? '#fd6531' : '#444444'" size="28"></uni-icons>
					</uni-badge>
				</view>
				<view @click="clickFavorite">
					<uni-badge :offset="[0, 12]" :text="topic.fav_count.toString()" :inverted="!topic.is_favorite" type="error" absolute="rightTop">
						<uni-icons :type="topic.is_favorite == 1 ? 'star-filled' : 'star'" :color="topic.is_favorite == 1 ? '#fd6531' : '#444444'" size="28"></uni-icons>
					</uni-badge>
				</view>
			</view>
		</view>
		...
		<l-painter v-if="isShowPainter" isRenderImage custom-style="position: fixed; left: 200%;" :board="base" @success="onPainterSuccess" />
		<!-- #endif -->
...
export default {
	...
	onShow() {
		if (!this.topic) {
			...
			// #ifdef MP-BAIDU
			let content = this.topic.content.replace(/(<([^>]+)>)/gi, '');
			swan.setPageInfo({
	...
	methods: {
		 ...
		 */
		contentText() {
			let text = this.topic.content.replace(/(<([^>]+)>)/gi, '');
