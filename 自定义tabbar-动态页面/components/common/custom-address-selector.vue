...
@click="anewSelect(0)"
>{{ provinceObj.name || "请选择" }}</text
>
<text
v-if="showIndex == 1 || cityObj.name"
:class="showIndex == 1 ? 'select' : ''"
@click="anewSelect(1)"
>{{ cityObj.name || "请选择" }}</text
>
<text
v-if="showIndex == 2 || areaObj.name"
:class="showIndex == 2 ? 'select' : ''"
@click="anewSelect(2)"
>{{ areaObj.name || "请选择" }}</text
>
<text
v-if="(showIndex == 3 || streetObj.name) && selectAreaLevelLimit == 4"
:class="showIndex == 3 ? 'select' : ''"
@click="anewSelect(3)"
>{{ streetObj.name || "请选择" }}</text
>
</view>
<!-- 待选择的地址信息 -->
...
// this.$apiUrl.storeArea:请求路径 每次请求带上parentArea 即可请求下一级区域数据 接口根据自身情况来决定
	console.log("请求地址参数",data);
	this.$u.api.address.getAddressList(data).then(resp => {
uni.hideLoading();
		console.log("请求地址列表响应结果",resp);
if (resp.success) {
