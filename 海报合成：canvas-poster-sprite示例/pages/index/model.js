...
var proData = {
	qrcodeUrl : 'https://img.xialv.com/static/ticket-h5/popup/qrcode.png',
	iconUrl : 'https://static4.xialv.com/img/img/icon.jpeg',
...
function getHotParam(param, callback) {
	...
	var topBg = "https://static4.xialv.com/img/img/hot_top.jpg";//头部图片
	var topHeight = 500;//头部图片高度

	pics.push({x : 0, y : 0, preload : true, src : topBg});
	
	//uni不支持Dom操作，写个简单的measureText函数代替
	// //新建canvas元素-测量文字宽度
	...
	var bPic = {
		x : 0,
		y : bTop,
		src : "https://static4.xialv.com/img/img/hot_bottom.jpg"
	}
	//qrcode
	var qrTop = bTop;
	var qrLeft = 550;
...
//简单测量文字的宽度
function measureText(str, charSize = 6){
	if(!str) return 0;
	return str.toString().length * charSize;
}

export function getCanvasParam(){
	return getHotParam(proData);
}