  ...
  <u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="value" length="auto"
    ...
    <view class="u-datetime-picker">
      <view class="u-picker-header" @touchmove.stop.prevent="">
        ...
        <view class="u-btn-picker u-btn-picker--tips cancelColor" hover-class="u-opacity" :hover-stay-time="150" @tap="getResult('cancel')">
          {{cancelText}}</view>
        <view class="u-picker__title">{{ title }}</view>

        <!-- :style="{ color: moving ? cancelColor : confirmColor }" -->
        <view class="u-btn-picker u-btn-picker--primary" hover-class="u-opacity" :class="[moving ? 'cancelColor' : 'confirmColor']"
      ...
      <view class="u-picker-body">
        ...
        <picker-view v-else-if="mode == 'time'" :value="valueArr" @change="change" class="u-picker-view" @pickstart="pickstart"
          ...
          <picker-view-column v-if="!reset && params.year">
            <view class="u-column-item" v-for="(item, index) in years" :class="[(year==item?'u-column-item-active':'')]" :key="index">
              {{ item }}
              <text class="u-text" v-if="showTimeTag">年</text>
            </view>
          </picker-view-column>
          <picker-view-column v-if="!reset && params.month">
            <view class="u-column-item" v-for="(item, index) in months" :class="[(month==item?'u-column-item-active':'')]" :key="index">
              {{ formatNumber(item) }}
        ...
        <picker-view v-else-if="mode == 'selector'" :value="valueArr" @change="change" class="u-picker-view" @pickstart="pickstart"
          ...
          <picker-view-column v-if="!reset">
            <view class="u-column-item" v-for="(item, index) in range" :key="index">
              <view class="u-line-1" :class="[(valueArr[0]==index?'u-column-item-active':'')]">{{ getItemValue(item, 'selector') }}</view>
            </view>
          </picker-view-column>
        </picker-view>
        <picker-view v-else-if="mode == 'multiSelector'" :value="valueArr" @change="change" class="u-picker-view" @pickstart="pickstart"
          @pickend="pickend">
          <picker-view-column v-if="!reset" v-for="(item, index) in range" :key="index">
            <view class="u-column-item" v-for="(item1, index1) in item" :key="index1">
              <view class="u-line-1" :class="[(valueArr[index]==index1?'u-column-item-active':'')]">
...
export default {
  ...
  methods: {
    ...
    initTimeValue() {
      ...
      // let fdate = this.defaultTime.replace(/\-/g, '/');   // 苹果默认值无法初始化处理注释
      let fdate = this.defaultTime
      // fdate = fdate && fdate.indexOf('/') == -1 ? `2020/01/01 ${fdate}` : fdate; // 苹果默认值无法初始化处理注释
      let time = null
