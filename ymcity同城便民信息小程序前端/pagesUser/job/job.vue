		...
		<z-paging ref="paging" @query="queryList" :list.sync="listData" :style="{height: scrollHeight+'px'}">
			...
			<view>
				<view class="list" v-for="(item,index) in listData" :key="index">
					<view class="titbox" @click="$u.route('/pagesIndex/jobmain/jobmain',{id:item.id})">
						<u-icon v-if="item.is_top=='2'" name="/static/images/top.png" size="40"
							style="margin-right: 10rpx;"></u-icon>
						<text class="title">{{item.title}}</text>
					</view>
					<text class="type" @click="$u.route('/pagesIndex/jobmain/jobmain',{id:item.id})">{{item.type_text}}</text>
					<view class="cell" @click="$u.route('/pagesIndex/jobmain/jobmain',{id:item.id})">
						<text class="price" v-if="item.is_face=='1'">面议</text>
						<text class="price" v-else>{{item.salary_min}}-{{item.salary_max}}元/月</text>
	...
	export default {
		...
		methods: {
			...
			queryList(pageNo, pageSize) {
				...
				this.$u.get('/addons/hicity/job/pushList', this.search).then(res => {
			...
			toTop(params) {
				this.$u.get('/addons/hicity/job/topInfo', params).then(res => {
			...
			downInfo(item, index) {
				this.$u.get('/addons/hicity/job/upInfoStatus', {
					id: item.id,
					type: 'down'
				}).then(res => {
			...
			topay(item){
				this.$u.get('/addons/hicity/job/topay', {
					id: item.id,
				}).then(res => {
					this.$u.route('/pagesPush/pay/pay', {
						order_no: res.order_no,
