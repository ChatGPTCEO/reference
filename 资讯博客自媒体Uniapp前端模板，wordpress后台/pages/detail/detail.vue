		...
		<view class="topic-detail">
			<view class="article-info" v-if="detail">
				...
				<view class="article-info__category">
					<text class="category">{{detail.meta.views}}浏览 · </text>
			...
			<view class="next-page" v-if="detail.next_post">
			   <text @tap="bindDetail(detail.next_post.id)" class="g-text-overflow next-page-bubble-show">{{detail.next_post.title.rendered}}</text>
			   <view @tap="bindDetail(detail.next_post.id)" class="next-page-icon">
			     <image class="icontext" src="/static/images/perv.png"></image>
			     <text>上一篇</text>
			   </view>
			 </view>
			
			 <view class="next-page" v-if="detail.prev_post">
			    <text @tap="bindDetail(detail.prev_post.id)" class="g-text-overflow next-page-bubble-show">{{detail.prev_post.title.rendered}}</text>
			    <view @tap="bindDetail(detail.prev_post.id)" class="next-page-icon">
			      <image class="icontext" src="/static/images/next.png"></image>
			      <text>下一篇</text>
			    </view>
			  </view>
			...
			<view class="commentdata">
				<view class="listnodata" v-if="comments.length==0">
					<image class="nodataimg" lazyLoad="false" mode="aspectFit|aspectFill|widthFix"
						src="@/static/images/empty.png"></image>
...
<style>
	...
	.next-page>text {
	  ...
	  line-height: 84rpx;
	  background: #ffffff;
	  box-shadow: 0 2rpx 20rpx 0rpx rgba(118, 118, 118, 0.2);
	  border-radius: 50rpx;
	  transition: color 0.2s, flex 0.3s ease 0.2s;
	}
	
	text.next-page-bubble-show {
	  flex: 0 0 670rpx;
	  padding-right: 84rpx;
	  color: #666666;
	  transition: flex 0.3s, color 0.2s ease 0.3s;
	}
