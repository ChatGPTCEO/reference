<template>
	<view class="u-p-30">
		...
		<view class="form">
			...
			<u-form-item label="分类" label-width="150rpx">
				<u-input v-model="label_text" type="select" @click="labelShow=true" placeholder="请选择" />
			</u-form-item>
			<u-form-item label="图片" prop="photo" label-width="150">
			...
			<u-form-item label="联系电话" label-width="150rpx">
				<u-input v-model="form.person_mobile" placeholder="请输入" type="number"/>
			</u-form-item>
		...
		<u-select v-model="labelShow" :list="labelList" value-name="id" label-name="name" @confirm="confiremLabel">
		</u-select>
		...
		methods: {
			getCateFreeNum() {
				this.$u.get('/addons/hicity/user/getCateFreeNum', {
					...
					this.freenum = res.vipConfig.promote_free_num;
			...
			getLabels() {
				this.$u.get('/addons/hicity/promote/getLabels', {
			...
			confiremLabel(e) {
				this.form.label_id = e[0].value;
				this.label_text = e[0].label;
			},
			openTop() {
			...
			submit() {
				...
				this.$u.post('/addons/hicity/promote/add', this.form).then(res => {
					if (res.status == '1') {
						this.$u.route({
							type: 'redirectTo',
