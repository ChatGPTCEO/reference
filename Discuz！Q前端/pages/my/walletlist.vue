    ...
    >
      <view class="walletlist-items" v-if="dataList.length > 0">
        <qui-cell-item
          ...
          :addon="
            item.change_type === 100 || item.change_type === 101
              ? item.change_freeze_amount >= 0
                ? `+￥${item.change_freeze_amount}`
                : `-￥${item.change_freeze_amount.substr(1)}`
      ...
      filterList: [
        {
          ...
          data: [
            ...
            { label: this.i18n.t('profile.redpacketpay'), value: [100, 101, 102, 103, 104] },
            { label: this.i18n.t('profile.longredpacketpay'), value: [110, 111, 112, 113, 114] },
            { label: this.i18n.t('profile.offerewardpay'), value: [120, 121, 122, 123, 124] },
          ],
        },
      ],
    };
    ...
    getList(type) {
      ...
      status
        ...
        .then(res => {
          ...
          const arr = Object.assign([], this.dataList);
          arr.forEach((item, index) => {
            if (item.change_desc === '文字帖红包冻结') {
              arr[index].change_freeze_amount = arr[index].change_available_amount;
            }
          });
          this.dataList = arr;
        });
