			...
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				<block v-if="page.type=='hierarchy'&& page.children.length>0">
					<scroll-view class="sub-menu-list left" :class="[activeMenuArr[page_index].length>1?'first':'alone']"
					 :scroll-y="true" :scroll-into-view="'firstid'+firstScrollInto">
					...
					if (typeof(item[0]) == 'object') {
						...
						item.forEach((s, j) => {
							if(s!=null){
								...
								item[j] = s;
								s.forEach((v, k) => {
									value[i][j][k] = (v==null||v>=this.subData[i].children[j].children.length)?null:this.subData[i].children[j].children[v].id;
