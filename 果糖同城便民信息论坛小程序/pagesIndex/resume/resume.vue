		...
		<z-paging ref="paging" @query="queryList" :list.sync="dataList" :style="{height: scrollHeight+'px'}">
			<view slot="top">
				<u-dropdown style="background-color: white;" ref="uDropdown" active-color="#f44c01">
					<u-dropdown-item v-model="search.edu" title="学历" :options="options1" @change="refload">
					</u-dropdown-item>
					...
					<u-dropdown-item title="更多">
						<view class="slot-content">
							<view class="slot-cell">
								<text>性别</text>
								<view class="gird">
									<view class="gitem" v-for="(item,index) in sextag" :key="index"
										@click="seltag('sex',item)"
							...
							<view class="slot-cell">
								<text>年龄</text>
								<view class="gird">
									<view class="gitem" v-for="(item,index) in agetag" :key="index"
										@click="seltag('age',item)" :class="{active:search.age==item.value?true:false}">
										<text>{{item.label}}</text>
									</view>
								</view>
							</view>
