<template>
	<view class="w-100" :style="commonWindowHeight" style="background-color: rgba(178,0,0,0.6);">
		<view class="w-750 row" :style="commonWindowHeight" style="touch-action: none">
			<video :src="articleDetail.video" autoplay :controls="false" class="w-100"
				:poster="articleDetail.videoCover" object-fit="contain" loop :style="commonWindowHeight"
				enable-play-gesture show-mute-btn controls />
		</view>
		...
		<view class="foot-box flex-1 text-white">
			...
			<view class="flex-1 mx-3">
				<view class="flex-1 product-box row-ac"  v-for="(item,index) in articleDetail.productData" :key="index"
					v-if="index < 1" @click="toProduct(item)">
		...
		<window-popup @clicks-witch="clicksWitchEmit" :popupHeight="1100">
			<view class="article-comment flex-1 mx-3">
				<view class="font-s-3 pb-3">共{{ articleDetail.commentCount }}条评论</view>
										...
										<view class="d-f">
											<view class="flex-1 mr-5">
												<view class="font-s-3 text-light-grey">
													{{ childrenCommentItem.username }}</view>
												<view class="font-s-2 mt" @click="showComment(item)">
											...
											<view class="d-f flex-column align-center content-comment "
												...
												<view class="font-s-1" v-if="childrenCommentItem.fabulousCount > 0">
													{{ childrenCommentItem.fabulousCount }}</view>
											</view>
										</view>
									</view>
								</view>
			...
			<view class="row-ac fixed-bottom comment-input-box flex-1" v-if="commentForm.commentStatus">
				...
				<view class="release-cancel mb-2 row mx-2 font-s-3 bg-warning text-white" @click="cancleComment">取消</view>
			</view>
		...
		created() {
			uni.getSystemInfo({
				success: res => {
					...
					this.appVideoHeight = this.windowHeight - this.statusBarHeight - uni.upx2px(438)
				}
			});
		},
		computed: {
