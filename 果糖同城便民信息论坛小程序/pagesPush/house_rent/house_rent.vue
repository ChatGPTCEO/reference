<template>
	<view class="u-p-30">
		...
		<view class="form">
			...
			<u-form-item label="租金" label-width="150rpx">
				<view style="display: flex;flex-direction: row;align-items: center;flex: 1;">
			...
			<u-form-item label="房屋配置" label-width="150rpx">
				<view class="rowcell" @click="showConfig()">
					<text style="color: rgb(192, 196, 204);flex: 1;" v-if="config_names.length===0">请选择</text>
					<block v-else>
						<view class="cell">
							<text class="item" v-for="(item,index) in config_names" :key="index">{{item}}</text>
						</view>
					</block>
		...
		<mulselect ref="configSelect" :listData="configData" :is_mult="true" @confirem="confiremConfig"></mulselect>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.getTopConfig()
			this.getLabels('house_rent_label')
			this.getLabels('house_config')
			this.getCateFreeNum()
		},
		methods: {
			getCateFreeNum() {
			...
			confiremConfig(e) {
				this.form.config_ids = e.ids.join(',');
				this.config_names = e.labels;
			},
