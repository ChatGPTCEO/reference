			...
			<view class='header bg-color'>
				<view class='headerCon acea-row row-between-wrapper'>
					...
					<navigator class='right acea-row row-middle' hover-class='none'
						url='/pages/user/integral/sign_list'>
			...
			<view class='wrapper wrapper2'>
				...
				<view class='list3'>
					<view class='item acea-row' v-for="(item,index) in signList" :key="index">
						<view>
							<view class='name line1'>{{item.integral_title}}</view>
							<view class='data'>{{item.create_time}}</view>
						</view>
						<view class='num' style="color: #F43F3B;">+{{item.integral_change}}</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			getSignSysteam: function() {
				let that = this;
				that.$u.get('/api/sign/config').then(res => {
			 ...
			 */
			goSignList: function() {
				return this.$util.Tips('/pages/user/integral/sign_list');
			},
			/**
			 * 获取用户信息
			 */
			getUserInfo: function() {
				let that = this;
				that.$u.post('/api/sign/user', {
					userid: that.UserInfo.uid
				}).then(res => {
					res.data.integral = parseInt(res.data.integral);
					let sum_sign_day = res.data.sum_sign_day;
					that.$set(that, 'userInfo', res.data);
					that.signCount = that.PrefixInteger(sum_sign_day, 4);
					that.sign_index = res.data.sum_sign_day;
				})
			},
