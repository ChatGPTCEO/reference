<template>
	<view class="flex-1 d-f justify-between" :style="modulMTop">
		<view class="article-box">
			<view class="article-item flex-1" :style="itemBox" v-for="(item,index) in articleList.leftArr" :key="index"
				@click="toArticleDetails(item)" v-if="index < quantityUum">
				<view class="flex-1 img-box">
					<view class="flex-1" :class="articleInfo.waterfallFlow == 1 ? 'article-img' : ''">
						<image class="flex-1" :style="imgRadius" :src="item.image[0] || item.videoCover "
							:mode="articleInfo.waterfallFlow == 1 ? 'aspectFill' : 'widthFix'"></image>
					</view>
					...
					<view class="article-info text-white py-2"
						:class="articleInfo.waterfallFlow == 0 ? 'gradient-mt' : 'gradient-mt-1'"
						v-if="articleInfo.showProduct == 0">
				...
				<view class="flex-1 mx-2 pb-2 pt-1 foot-box"
				...
				<view class="flex-1 mx-2 pb-2 foot-box" v-else>
					<view class="font-s-3 mt-1 in1line" :style="fontSize">{{ item.title }}</view>
		...
		<view class="article-box">
			<view class="article-item flex-1" :style="itemBox" v-for="(item,index) in articleList.rightArr" :key="index"
				@click="toArticleDetails(item)" v-if="index < quantityUum">
				<view class="flex-1 img-box">
					<view class="flex-1" :class="articleInfo.waterfallFlow == 1 ? 'article-img' : ''">
						<image class="flex-1" :style="imgRadius" :src="item.image[0] || item.videoCover"
...
<script>
	...
	export default {
		...
		created() {
			...
			this.quantityUum = this.articleInfo.articleList.quantity / 2
		},
		methods: {
			initData() {
				this.getArticleList()
