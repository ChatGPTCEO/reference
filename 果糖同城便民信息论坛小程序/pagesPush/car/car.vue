<template>
	<view>
		...
		<view class="form">
			...
			<u-form-item label="上牌时间" label-width="150rpx">
				<u-input v-model="reg_time_text" @click="regtimeShow=true" type="select" placeholder="请选择" />
			</u-form-item>
			<u-form-item label="行驶里程" label-width="150rpx">
				<view style="display: flex;flex-direction: row;align-items: center;flex: 1;">
					<u-input style="flex: 1;" v-model="form.mileage" placeholder="请输入" />
					<text>万公里</text>
				</view>
			</u-form-item>
			<u-form-item label="售价" label-width="150rpx">
				<view style="display: flex;flex-direction: row;align-items: center;flex: 1;">
					<u-input style="flex: 1;" v-model="form.price" placeholder="请输入" />
					<text>万元</text>
				</view>
			</u-form-item>
			...
			<u-form-item label="车辆图片" label-width="150">
			...
			<u-form-item label="车辆描述" label-width="200rpx" label-position="top">
				<u-input v-model="form.content" type="textarea" height="200" :maxlength="-1" placeholder="请输入车辆描述" />
			</u-form-item>
		...
		<u-picker v-model="regtimeShow" mode="time" :params="params" @confirm="confiremRegtime"></u-picker>
		...
		methods: {
			getCateFreeNum() {
				this.$u.get('/addons/hicity/user/getCateFreeNum', {
					...
					this.freenum = res.vipConfig.car_free_num;
			...
			confiremRegtime(e) {
				this.form.reg_time = e.timestamp;
				this.reg_time_text = e.year + '-' + e.month;
			},
			submit() {
				if (this.form.title == '') {
					return this.$u.toast('请输入标题')
				}
				if (this.form.reg_time == '') {
					return this.$u.toast('请选择上牌时间')
				}
				if (this.form.mileage == '') {
					return this.$u.toast('请输入行驶里程')
				}
				if (this.form.price == '') {
					return this.$u.toast('请输入售价')
				}
				if (this.form.banner_images == '') {
					return this.$u.toast('请上传车辆图片')
				}
				if (this.form.content == '') {
					return this.$u.toast('请输入车辆描述')
				}
				...
				this.$u.post('/addons/hicity/car/add', this.form).then(res => {
					if (res.status == '1') {
						this.$u.route({
							type: 'redirectTo',
