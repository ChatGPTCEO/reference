		...
		<z-paging ref="paging" @query="queryList" :list.sync="listData" :style="{height: scrollHeight+'px'}">
			...
			<view>
				<view class="list" v-for="(item,index) in listData" :key="index">
					<view class="titbox" @click="$u.route('/pagesIndex/askmain/askmain',{id:item.id})">
						<u-icon v-if="item.is_top=='2'" name="/static/images/top.png" size="40"
							style="margin-right: 10rpx;"></u-icon>
						<text class="title">{{item.title}}</text>
					</view>
					<view class="cell" @click="$u.route('/pagesIndex/askmain/askmain',{id:item.id})">
						<text class="time">{{item.type_text}}</text>
	...
	export default {
		...
		methods: {
			...
			queryList(pageNo, pageSize) {
				...
				this.$u.get('/addons/hicity/ask/pushList', this.search).then(res => {
			...
			toTop(params) {
				this.$u.get('/addons/hicity/ask/topInfo', params).then(res => {
			...
			downInfo(item, index) {
				this.$u.get('/addons/hicity/ask/upInfoStatus', {
					id: item.id,
					type: 'down'
				}).then(res => {
			...
			topay(item){
				this.$u.get('/addons/hicity/ask/topay', {
					id: item.id,
				}).then(res => {
					this.$u.route('/pagesPush/pay/pay', {
						order_no: res.order_no,
