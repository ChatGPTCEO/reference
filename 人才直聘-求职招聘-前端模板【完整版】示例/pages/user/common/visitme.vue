<template>
	<yzb-page :loadStatus="loadStatus">
		<yzb-visitme  v-if="list.length>0" :list="list" @click="toDetail" @onDelete="toDelete"></yzb-visitme>
...
import yzbVisitme from '@/components/yzb/yzb-visitme.vue';
export default {
	computed: {
		...mapState(['userInfo']),
		...mapGetters(['hasLogin'])
	...
	methods: {
		async getList() {
			...
			this.status = '请求中';
			let res = await this.$apis.getVisitList(this.query);
			console.log("-------",res);
			if (res) {
				let data = res.data;
